...
/* ================================================================= */
/* DRAG & DROP TEAM MEMBER ORDERING — Admin subpages                 */
/* Two pages under Our Team: "Sort Staff" and "Sort Board"           */
/* ================================================================= */

/* One-time migration: copy team_order ACF values into menu_order so
   existing sort is preserved when switching to drag-and-drop ordering. */
add_action('admin_init', 'cfsloco_migrate_team_order_to_menu_order');
function cfsloco_migrate_team_order_to_menu_order() {
    if (get_option('cfsloco_team_menu_order_migrated')) {
        return;
    }
    ...
    foreach ($members as $post_id) {
        $team_order = (int) get_post_meta($post_id, 'team_order', true);
        wp_update_post(array(
            'ID'         => $post_id,
            'menu_order' => $team_order,
        ));
    }
    update_option('cfsloco_team_menu_order_migrated', true);
}

add_action('admin_menu', 'cfsloco_team_sort_admin_pages');
function cfsloco_team_sort_admin_pages() {
    add_submenu_page(
        ...
        'edit_posts',
        'sort-staff',
        'cfsloco_render_team_sort_page_staff'
    );
    add_submenu_page(
        'edit.php?post_type=team_member',
        ...
        'edit_posts',
        'sort-board',
        'cfsloco_render_team_sort_page_board'
    );
}

function cfsloco_render_team_sort_page_staff() {
    cfsloco_render_team_sort_page('staff', 'Staff Members');
}

function cfsloco_render_team_sort_page_board() {
    cfsloco_render_team_sort_page('board-of-directors', 'Board of Directors');
}

/**
 * Shared renderer for the drag-and-drop sort page.
 */
function cfsloco_render_team_sort_page($term_slug, $title) {
    $members = get_posts(array(
        'post_type'      => 'team_member',
        'posts_per_page' => -1,
        'orderby'        => 'menu_order',
    ...
    ?>
    <div class="wrap">
        <h1>Sort <?php echo esc_html($title); ?></h1>
        <p>Drag and drop to reorder. Changes are saved automatically.</p>

        <div id="team-sort-status" style="display:none;padding:8px 12px;margin:10px 0;border-radius:4px;"></div>

        <ul id="team-sortable" data-term="<?php echo esc_attr($term_slug); ?>" style="max-width:600px;">
            <?php foreach ($members as $i => $member) :
                $thumb = get_the_post_thumbnail_url($member->ID, 'thumbnail');
                $job   = get_field('team_job_title', $member->ID);
            ?>
            <li data-id="<?php echo esc_attr($member->ID); ?>"
                style="background:#fff;border:1px solid #ccd0d4;padding:10px 14px;margin:4px 0;cursor:grab;display:flex;align-items:center;gap:12px;border-radius:4px;">
                <span class="dashicons dashicons-menu" style="color:#999;flex-shrink:0;"></span>
                <?php if ($thumb) : ?>
                    <img src="<?php echo esc_url($thumb); ?>" style="width:40px;height:40px;border-radius:50%;object-fit:cover;flex-shrink:0;" alt="">
                <?php else : ?>
                    <span style="width:40px;height:40px;border-radius:50%;background:#e8e8e8;display:inline-block;flex-shrink:0;"></span>
                <?php endif; ?>
                <span>
                    <strong><?php echo esc_html($member->post_title); ?></strong>
                    <?php if ($job) : ?>
                        <br><small style="color:#666;"><?php echo esc_html($job); ?></small>
                    <?php endif; ?>
                </span>
            </li>
            <?php endforeach; ?>
    ...
    jQuery(function($) {
        $('#team-sortable').sortable({
            ...
            update: function() {
                var order = [];
                $('#team-sortable li').each(function(i) {
                    order.push($(this).data('id'));
                });
                var $status = $('#team-sort-status');
                $.post(ajaxurl, {
                    action: 'cfsloco_save_team_order',
                    nonce: '<?php echo wp_create_nonce('cfsloco_team_sort'); ?>',
                    order: order
                }, function(res) {
                    if (res.success) {
                        $status.text('Order saved.').css({background:'#d4edda',color:'#155724',border:'1px solid #c3e6cb'}).fadeIn().delay(1500).fadeOut();
                    } else {
                        $status.text('Error saving order.').css({background:'#f8d7da',color:'#721c24',border:'1px solid #f5c6cb'}).fadeIn().delay(3000).fadeOut();
                    }
                });
            }
        });
        // Highlight style for the drag placeholder
        $('<style>.ui-state-highlight{background:#f0f6fc;border:2px dashed #2271b1;height:60px;margin:4px 0;border-radius:4px;list-style:none;}</style>').appendTo('head');
    });
    </script>
    <?php
}

/* Enqueue jQuery UI Sortable on our sort pages */
add_action('admin_enqueue_scripts', 'cfsloco_team_sort_enqueue');
function cfsloco_team_sort_enqueue($hook) {
    if (strpos($hook, 'sort-staff') !== false || strpos($hook, 'sort-board') !== false) {
        wp_enqueue_script('jquery-ui-sortable');
    }
}

/* AJAX handler — save the new menu_order for each post */
add_action('wp_ajax_cfsloco_save_team_order', 'cfsloco_save_team_order');
function cfsloco_save_team_order() {
    check_ajax_referer('cfsloco_team_sort', 'nonce');
